...
const CONFIG = {
  YT_MAX_RESULTS: 25,
  API_KEYS: [
    "AIzaSyCzu-Mqx22V83ktalXksUnC1AhtZwzyb-0","AIzaSyBM-uvKMHe5GxNuMpWB45-RWVUGYOGwEyQ",
    "AIzaSyAd6JdvYn7YGMfSY9EaJtCEUGd11tKa6ZI","AIzaSyBr2nxeKaN1q07fMV59zrLEOQx9dzYBsMI",
    "AIzaSyBbnepAY-irFm35H7Qu0NrwISzLCThkBKM","AIzaSyAujlR4Gig8puLuzM-amckcwu5sbMRvIR0",
    "AIzaSyBiGJ9JeOdkrUI7x-qQHyrHpUJAxcwRTvI","AIzaSyC_UCUc3zcffX5_IOPFpqbJyXmUYxKOg9U",
    "AIzaSyC7FYbsVmGA0LnepHG7t_xPOR0mEkQ1jiE","AIzaSyBLTPa7EUAmnJZMq4sYBT97x4HY3--YHws",
    "AIzaSyDWAi-0_oqg5GHwEoE0_LnSLSV_nsfs1SI","AIzaSyBJ6zZI3BFvBd02EcdsJFE7dLdZ-f7RV9c",
    "AIzaSyD_9Flh18VT4hJ0OkEIS3TCECJ4vIOcfC0","AIzaSyC3_rIZ5deseDbte7auVOo7oUDjopEMaBg",
...
// onYouTubeIframeAPIReady: crea los dos players (visible + hidden)
// Cambios clave: host = youtube-nocookie.com, playerVars incluye origin y enablejsapi
  ...
  try {
    state.visiblePlayer = new YT.Player(dom.ytVisibleHolder, {
      ...
      playerVars: {
        ...
        showinfo: 0,
        enablejsapi: 1,
        origin: location.origin || window.location.hostname
      },
      events: {
...
// ================== UI / Player control (tu lógica, respetada) ==================
...
function updatePlayingIndicator() {
  ...
  document.querySelectorAll('.video-item-title').forEach(title => { title.classList.remove('text-blue-500', 'font-bold'); title.classList.add('font-medium'); });
  ...
  playingItems.forEach(item => {
    ...
    if (title) { title.classList.add('text-blue-500', 'font-bold'); title.classList.remove('font-medium'); }
  });
}

function renderVideoList(container, items, listType = 'search', listId = null) {
  if (!items || items.length === 0) { container.innerHTML = '<p class="text-gray-400 text-center">No se encontraron resultados.</p>'; return; }
...
function renderPlaylistList(container, playlists) {
  if (!playlists || playlists.length === 0) { container.innerHTML = '<p class="text-gray-400 text-center">No se encontraron playlists.</p>'; return; }
  container.innerHTML = playlists.map(pl => {
    const sn = pl.snippet || {};
    const playlistId = (pl.id && pl.id.playlistId) ? pl.id.playlistId : (typeof pl.id === 'string' ? pl.id : (pl.playlistId || ''));
    return `<div class="bg-gray-800 p-3 rounded cursor-pointer playlist-item" data-playlist-id="${playlistId}" title="${sn.title}"><img src="${sn.thumbnails?.medium?.url || sn.thumbnails?.default?.url || 'https://placehold.co/320x180/000/fff?text=?'}" class="w-full h-36 object-cover rounded mb-2"><div class="text-sm font-medium truncate">${sn.title}</div><div class="text-xs text-gray-400 truncate">${sn.channelTitle}</div></div>`;
  }).join('');
  container.querySelectorAll('.playlist-item').forEach(el => {
    el.addEventListener('click', async () => {
      const playlistId = el.dataset.playlistId;
      if (!playlistId) { dom.resultsContainer.innerHTML = `<p class="text-red-400">ID de playlist inválido.</p>`; return; }
...
function showPage(pageId) {
  ...
  else if (pageId === 'home') { dom.homeTitle.textContent = 'Inicio'; dom.resultsContainer.innerHTML = ''; toggleHomeSections(true); }
...
function updatePlayerUI(track) {
  ...
  const bgThumb = snippet.thumbnails?.maxres?.url || snippet.thumbnails?.high?.url || snippet.thumbnails?.medium?.url || thumb;

  if (dom.playerBgImage) dom.playerBgImage.style.backgroundImage = `url('${bgThumb}')`;
...
function playTrack(video, queue = [], queueIndex = 0) {
  ...
  if (!state.playersReady) { state.queuedTrack = { video, queue, queueIndex }; updatePlayerUI(video); showFullPlayer(); setPlayIcon(true); return; }
  state.currentTrack = video; state.currentQueue = queue; state.currentQueueIndex = queueIndex; state.queuedTrack = null; addToRecents(video); loadAndPlayById(videoId, true); updatePlayerUI(video); if (!document.hidden) showFullPlayer(); updatePlayingIndicator();
}

function togglePlayPause() {
  if (!state.playersReady && state.queuedTrack) { const { video, queue, queueIndex } = state.queuedTrack; playTrack(video, queue, queueIndex); return; }
  const active = getActivePlayer();
...
function playNext() {
  ...
  if (state.isLoop) { playTrack(state.currentTrack, state.currentQueue, state.currentQueueIndex); return; }
  ...
  try {
    if (!document.fullscreenElement) {
      ...
      if (screen.orientation && screen.orientation.lock) {
        await screen.orientation.lock('landscape').catch(err => { console.log("No se pudo bloquear la orientación a horizontal:", err); });
      }
    } else {
      if (document.exitFullscreen) await document.exitFullscreen();
      if (screen.orientation && screen.orientation.unlock) { screen.orientation.unlock(); }
    }
  } catch(e) { console.error("Error al cambiar a pantalla completa:", e); showToast("La pantalla completa no es compatible."); }
}

// ========== Local storage helpers (tu código) ==========
...
function openLibraryOptionsModal() {
  ...
  const backdrop = document.createElement('div'); backdrop.className = 'modal-backdrop'; backdrop.id = 'library-options-backdrop';
  ...
  if (!state.libraries.length) { content.innerHTML = `<p class="text-sm text-gray-300">No hay bibliotecas locales. Debes crear una.</p>`; addBtn.classList.add('hidden'); } else { const select = document.createElement('select'); select.className = 'w-full bg-gray-700 text-white rounded p-2'; select.id = 'modal-library-select'; select.innerHTML = state.libraries.map(lib => `<option value="${lib.id}">${lib.name} (${lib.items.length})</option>`).join(''); content.appendChild(select); }
  ...
  modal.querySelector('#modal-create-confirm').addEventListener('click', () => { const input = modal.querySelector('#modal-create-library-input'), name = input.value.trim(); if (name) { const newLib = createLibrary(name); if (newLib) addCurrentTrackToLibrary(newLib.id); close(); } else { showToast('Ingresa un nombre válido.'); } });
  modal.querySelector('#modal-add-to-lib-btn').addEventListener('click', () => { if (!state.libraries.length) return; const selectEl = document.getElementById('modal-library-select'); if (!selectEl) return; addCurrentTrackToLibrary(selectEl.value); close(); });
}
...
function handleVisibilityChange() {
  ...
  if (!videoId) return;
  let source, target;
  if (document.hidden) { source = state.visiblePlayer; target = state.hiddenPlayer; } else { source = state.hiddenPlayer; target = state.visiblePlayer; }
  try {
    const srcState = (source && source.getPlayerState) ? source.getPlayerState() : -1;
    const isPlaying = (srcState === YT.PlayerState.PLAYING), isPaused = (srcState === YT.PlayerState.PAUSED);
    if (isPlaying || isPaused) {
      ...
      if (target && target.loadVideoById) {
        ...
        else setTimeout(() => { try { target.pauseVideo(); } catch(e) {} }, 150);
      }
      setTimeout(() => { try { if(source && source.stopVideo) source.stopVideo(); } catch(e) {} }, 300);
      setTimeout(() => { state.transferInProgress = false; }, 400);
    }
  } catch (e) { console.error("Error en handleVisibilityChange:", e); state.transferInProgress = false; }
}

// ================= INIT APP =================
function initApp() {
    // Event listeners principales
    dom.openSidebarBtn && dom.openSidebarBtn.addEventListener('click', () => toggleSidebar(true));
    dom.closeSidebarBtn && dom.closeSidebarBtn.addEventListener('click', () => toggleSidebar(false));
    dom.sidebarBackdrop && dom.sidebarBackdrop.addEventListener('click', () => toggleSidebar(false));
    ...
    dom.searchButton && dom.searchButton.addEventListener('click', () => { const query = dom.searchInput.value.trim(); if (query) { fetchVideosByQuery(query); showPage('home'); } });
    dom.searchInput && dom.searchInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') dom.searchButton.click(); });
    dom.searchPlaylistsButton && dom.searchPlaylistsButton.addEventListener('click', () => { const query = dom.searchInput.value.trim() || 'Top playlists'; fetchPlaylistsByQuery(query); showPage('home'); });
    ...
    dom.minimizePlayerBtn && dom.minimizePlayerBtn.addEventListener('click', ()=>showMiniPlayer());
    dom.maximizePlayerBtn && dom.maximizePlayerBtn.addEventListener('click', ()=>showFullPlayer());
    dom.playPauseBtn && dom.playPauseBtn.addEventListener('click', togglePlayPause);
    dom.miniPlayPauseBtn && dom.miniPlayPauseBtn.addEventListener('click', togglePlayPause);
    dom.nextBtn && dom.nextBtn.addEventListener('click', playNext);
    dom.prevBtn && dom.prevBtn.addEventListener('click', playPrev);
    dom.shuffleBtn && dom.shuffleBtn.addEventListener('click', toggleShuffle);
    ...
    dom.playlistBackBtn && dom.playlistBackBtn.addEventListener('click', () => { showPage(state.lastPage || 'home'); });
    dom.playlistPage && dom.playlistPage.addEventListener('scroll', () => { dom.playlistPage.classList.toggle('scrolled', dom.playlistPage.scrollTop > 150); });
    ...
    // Gestos del panel de cola (touch)
    if (dom.queueTriggerArea) {
    ...
    if (dom.playerQueueHandle) {
      ...
      dom.playerQueueHandle.addEventListener('touchend', () => {
        if (state.touchMoveY > 0 && state.touchMoveY - state.touchStartY > 50) { hideQueuePanel(); }
        state.touchStartY = 0; state.touchMoveY = 0;
      });
    }

    dom.fullscreenBtn && dom.fullscreenBtn.addEventListener('click', toggleFullScreen);

    loadFavorites(); loadLibraries(); loadRecents();
    fetchPopularMusic(); fetchPlaylistsByQuery('Top music');

    // Registrar SW si está presente (opcional)
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/sw.js').then(reg => {
        // registrado correctamente (diagnóstico)
        console.log('SW registrado:', reg.scope);
      }).catch(()=>{ /* SW podría no existir en GH Pages si no lo subes */ });
    }
}
...
/* ================== Diagnostics: detectar recursos tipo anuncios ================== */

/**
 * detectAdResourcesReport:
 * usa performance.getEntriesByType('resource') para buscar recursos con dominios típicos de ads.
 * Esto es solo DIAGNÓSTICO: te ayudará a comparar GitHub Pages vs AppCreator24.
 */
function detectAdResourcesReport() {
  try {
    if (!(performance && performance.getEntriesByType)) return;
    const resources = performance.getEntriesByType('resource') || [];
    const urls = resources.map(r => r.name || '');
    const adHits = urls.filter(u => /doubleclick|googlesyndication|googlead|adservice|pagead|imasdk|admanager|googletagservices/i.test(u));
    if (adHits.length) {
      console.warn('Detected ad-like network resources (performance):', adHits.slice(0, 30));
    } else {
      console.log('No ad-like resources detected in performance entries (yet).');
    }
    // enviar al SW o backend si quieres guardar el diagnóstico
    if (navigator.serviceWorker && navigator.serviceWorker.controller) {
      try {
        navigator.serviceWorker.controller.postMessage({ type: 'PERF_AD_CHECK', adHits: adHits.slice(0,30) });
      } catch(e){}
    }
  } catch(e) {
...
// Ejecutar diagnóstico en dos momentos
setTimeout(detectAdResourcesReport, 1500);
setTimeout(detectAdResourcesReport, 6000);

// También un pequeño logger de entorno para comparar (GH Pages vs AppCreator24)
function logEnvironment() {
  try {
    console.groupCollapsed('ENV DEBUG');
    ...
    console.log('display-mode standalone?', window.matchMedia && window.matchMedia('(display-mode: standalone)').matches);
    console.log('document.hidden =', document.hidden);
    console.groupEnd();
  } catch(e){}
}
setTimeout(logEnvironment, 800);
